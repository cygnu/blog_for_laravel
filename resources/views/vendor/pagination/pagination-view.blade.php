    ...
    <ul class="pagination" role="navigation">
        ...
        {{-- Pagination Elemnts --}}
            {{-- Array Of Links --}}
                <!-- それぞれのページへのリンク -->
                {{-- 定数よりもページ数が多い時 --}}
                @if ($paginator->lastPage() > $paginate)

                    {{-- 現在ページが表示するリンクの中心位置よりも左の時 --}}
                    @if ($paginator->currentPage() <= floor($paginate / 2))
                        <?php $start_page = 1; //最初のページ ?> 
                        <?php $end_page = $paginate; ?>

                    {{-- 現在ページが表示するリンクの中心位置よりも右の時 --}}
                    @elseif ($paginator->currentPage() > $paginator->lastPage() - floor($paginate / 2))
                        <?php $start_page = $paginator->lastPage() - ($paginate - 1); ?>
                        <?php $end_page = $paginator->lastPage(); ?>

                    {{-- 現在ページが表示するリンクの中心位置の時 --}}
                    @else
                        <?php $start_page = $paginator->currentPage() - (floor(($paginate % 2 == 0 ? $paginate - 1 : $paginate)  / 2)); ?>
                        <?php $end_page = $paginator->currentPage() + floor($paginate / 2); ?>
                    @endif
